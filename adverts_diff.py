        ...
        for i in range(3):
            if id==int(shared_list[i]["Ad ID"]["S"]):
                ...
                if int(shared_list[i]["Clicks"]["N"])- int(shared_list[i]["Sales"]["N"]) > 3 :
                    ...
                    print("Ad ID: ",id ," Revenues: ",int(shared_list[i]["Sales"]["N"])*prod_sale," Cost: ",int(shared_list[i]["Clicks"]["N"])*bid," Profit: ",int(shared_list[i]["Sales"]["N"])*prod_sale-int(shared_list[i]["Clicks"]["N"])*bid," Increase: ",new_bid-bid," From: ",bid," To: ",new_bid)
